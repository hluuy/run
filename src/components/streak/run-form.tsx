...
export function RunForm({ onSuccess }: RunFormProps) {
  ...
  const {
    ...
    defaultValues: { date: todayKST(), hours: 0, minutes: 0, seconds: 0, avg_heart_rate_bpm: null },
  })
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 pb-4">
      ...
      <div className="space-y-1.5">
        <Label>
          GPX 파일 <span className="text-muted-foreground">(선택 — 자동으로 스탯 입력)</span>
        </Label>
        {gpxFile ? (
          <div className="flex items-center gap-2 rounded-lg border border-border bg-muted px-3 py-2">
            ...
            {gpxAutoFilled && (
              <span className="text-xs text-orange-500 shrink-0">자동 입력됨</span>
            )}
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-2">
          ...
          ].map(({ field, max, placeholder, label }, i) => (
            <div key={field} className="flex-1">
              {i > 0 && <span className="absolute -left-3 top-1/2 -translate-y-1/2 text-muted-foreground">:</span>}
